...
function _parseVideoInitialData(responseText, callback) {
  ...
  var description =_jp.query(ipdata, '$..description..simpleText').join('') || _jp.query(ipdata, '$..microformat..description..simpleText').join('') || _jp.query(ipdata, '$..videoDetails..shortDescription').join('');

  var author_name = _jp.value(idata, '$..owner..title..text') || _jp.value(idata, '$..owner..title..simpleText');

  var author_url = _jp.value(ipdata, '$..microformat..playerMicroformatRenderer..ownerProfileUrl') || _jp.value(idata, '$..owner..title..url');
  ...
  var channelId = _jp.value(ipdata, '$..videoDetails..channelId') || _jp.value(ipdata, '$..microformat..playerMicroformatRenderer..externalChannelId');

  var channelUploadsPlaylistId;
  if (channelId.substring(0, 2) == "UC") {
    channelUploadsPlaylistId = channelId.replace("UC", "UU");
  } else {
    channelUploadsPlaylistId = undefined;
  }
  ...
  var video = {
    ...
    videoId: videoId,
    channelId: channelId,
    channelUploadsPlaylistId: channelUploadsPlaylistId,
    seconds: Number(duration.seconds),
    timestamp: duration.timestamp,
    duration: duration,
...
function _parsePlaylistInitialData(responseText, callback) {
  ...
  list.forEach(function (playlistVideoRenderer) {
    ...
    var video = {
      ...
      listId: listId,
      thumbnail: (_jp.value(json, '$..thumbnail..thumbnails[0]..url') === undefined ? "" : _jp.value(json, '$..thumbnail..thumbnails[0]..url').split('?')[0]),
